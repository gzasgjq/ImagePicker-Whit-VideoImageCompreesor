...
public class TrimVideoUtils {
    ...
    public Flowable<TrimResult> startTrimRx(final boolean isNew, final int startS, final int endS, final File file, final File trimFile) {
        return Flowable.create(new FlowableOnSubscribe<TrimResult>() {
            @Override
            public void subscribe(FlowableEmitter<TrimResult> emitter) throws Exception {
                // 需要裁剪的视频必须存在
                if (file != null && file.exists()) {
                    try {
                        ...
                        // --
                        if (isStopTrim) {
                            if (!trimFile.getParentFile().exists())
                                trimFile.getParentFile().mkdirs();
                            if (trimFile.exists()) {
                                trimFile.delete();
                            }
                            emitter.onComplete();
                        ...
                        result.file = file;
                        result.trimFile = trimFile;
                        emitter.onNext(result);
                    } catch (Exception e) {
                        e.printStackTrace();
                        // --
                        emitter.onError(new IllegalStateException("TRIM_FAIL"));
                        // --
                        try {
                            if (trimFile.exists()) {
                                trimFile.delete();
                    ...
                    emitter.onError(new IllegalStateException("FILE_NOT_EXISTS"));
                }
            }
        }, BackpressureStrategy.DROP).doOnError(new Consumer<Throwable>() {
            @Override
            public void accept(Throwable throwable) throws Exception {
                Log.e(TrimVideoUtils.class.getSimpleName(), "accept: ", throwable);
            }
        });
    }
