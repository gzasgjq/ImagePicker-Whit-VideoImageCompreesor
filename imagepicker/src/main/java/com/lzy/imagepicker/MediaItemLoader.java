...
public class MediaItemLoader extends AsyncTaskLoader<ArrayList<ImageFolder>> {
    ...
    public ArrayList<ImageFolder> loadInBackground() {
        ...
        try {
            ...
            switch (mMediaType) {
                case ImagePicker.MediaType.MEDIA_MIX:
                    datas = getMixData();
                    break;
                case ImagePicker.MediaType.MEDIA_VIDEO:
                    datas = getVideoData();
                    break;
                case ImagePicker.MediaType.MEDIA_IMAGE:
                default:
                    datas = getImageData();
                    break;
            }
    ...
    private ArrayList<ImageFolder> getMixData() {
        ...
        if (mVideoCursor != null && mImageCursor != null) {
            if (!mVideoCursor.moveToFirst() && !mImageCursor.moveToFirst()) return null;
            ...
            imageFolders.clear();
            ArrayList<ImageItem> allMedias = new ArrayList<>();   //所有的集合,不分文件夹
            ArrayList<ImageItem> allVideos = new ArrayList<>();   //所有视频的集合,不分文件夹
            ...
            if (mixCursor.getCount() > 0) {
                ...
                if (allMedias.size() > 0) {
                    ...
                    imageFolders.add(0, allImagesFolder);  //确保第一条是所有
                }
            }
            mVideoCursor.close();
            mImageCursor.close();
...
//        }
    ...
    public MediaItemLoader(@NonNull Context context, @ImagePicker.MediaType int mediaType,
...
//    }
    ...
    public @ImagePicker.MediaType
    int getmMediaType() {
        return mMediaType;
    }

    public void setmMediaType(@ImagePicker.MediaType int mMediaType) {
