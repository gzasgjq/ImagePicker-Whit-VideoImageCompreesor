...
public class ImagePreviewActivity extends ImagePreviewBaseActivity implements ImagePicker.OnImageSelectedListener, View.OnClickListener, CompoundButton.OnCheckedChangeListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (item.mimeType.startsWith("video") &&
                !(MediaUtil.getMediaInfo(item.path) != null &&
                MediaUtil.getMediaInfo(item.path).getString(MediaFormat.KEY_MIME).equals(MediaFormat.MIMETYPE_VIDEO_AVC))) {
            bottomBar.setVisibility(View.GONE);
            noSupportHint.setVisibility(View.VISIBLE);
        }else {
            bottomBar.setVisibility(View.VISIBLE);
            noSupportHint.setVisibility(View.GONE);
        }
        ...
        mCbCheck.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (MediaUtil.isFileValid(ImagePreviewActivity.this, mCbCheck.isChecked(), imageItem, selectedImages)) {
...
//                        Snackbar.make(content, ImagePreviewActivity.this.getString(R.string.selectvideo_limit, selectLimit), Snackbar.LENGTH_SHORT).show();
//                        mCbCheck.setChecked(false);
//                    } else {
//                        if (!new File(imageItem.path).exists()) {
//                            Snackbar.make(content, ImagePreviewActivity.this.getString(R.string.invalid_video), Snackbar.LENGTH_SHORT).show();
//                            mCbCheck.setChecked(false);
//                        } else {
...
//                    if (mCbCheck.isChecked() && selectedImages.size() >= selectLimit) {
////                    Toast.makeText(ImagePreviewActivity.this, ImagePreviewActivity.this.getString(R.string.select_limit, selectLimit), Toast.LENGTH_SHORT).show();
//                        Snackbar.make(content, ImagePreviewActivity.this.getString(R.string.select_limit, selectLimit), Snackbar.LENGTH_SHORT).show();
//                        mCbCheck.setChecked(false);
//                    } else {
//                        if (!new File(imageItem.path).exists()) {
//                            Snackbar.make(content, ImagePreviewActivity.this.getString(R.string.invalid), Snackbar.LENGTH_SHORT).show();
//                            mCbCheck.setChecked(false);
//                        } else {
//                            if (imagePicker.getSelectImageCount() > 0 && imagePicker.isSelectedImageContainsVideo()) {
//                                mCbCheck.setChecked(false);
...
//                }
    ...
    public void onImageSelected(int position, ImageItem item, boolean isAdd) {
        if (imagePicker.getSelectImageCount() > 0) {
            mBtnOk.setText(
                    getString(
                            R.string.select_complete
                            , imagePicker.getSelectImageCount()
                            , (item != null && item.mimeType.startsWith("video")) ? imagePicker.getSelectVideoLimit() : imagePicker.getSelectLimit()
                    )
            );
...
//            setResult(ImagePicker.RESULT_CODE_ITEMS, intent);
            checkCompress(imagePicker.getSelectedImages());
//            finish();
    ...
    protected void onCompressStart() {
        if (loadingDialog == null) {
            View v = getLayoutInflater().inflate(R.layout.view_loading, null);
            loadingDialog = new Dialog(this, R.style.AlertDialogStyle);
